...
<%@ page import="ogrg.zerock.webtest1.dao.ProgramDAO" %>
<%@ page import="ogrg.zerock.webtest1.dto.ProgramDTO" %>
<%@ page import="java.util.List" %>
<%@ page import="javax.servlet.http.HttpSession" %>
<%@ page import="static jdk.javadoc.internal.doclets.formats.html.markup.HtmlStyle.title" %>
<%
	ProgramDAO programDAO = new ProgramDAO();
	List<ProgramDTO> programList = programDAO.getAllPrograms();
		...
		<div class="bodytext_area place_area box_inner">
			<ul class="program_list clear">
				<%	for (ProgramDTO program : programList) {%>
					<li>
						<img class="img_place" src="/img/<%= program.getImg() %>" alt="<%= program.getTitle() %>" />
						<h3><%= program.getTitle() %></h3>
						<p class="subttl"><%= program.getSchedule() %></p>
						<div class="program_content">
							<p><%= program.getText() %> <span class="subtxt"><%= program.getSubrText() %></span></p>
						</div>
						<p class="btn_more"><a href="javascript:;">더보기</a>
							<a href="/saveTitle?title=<%= program.getTitle() %>">저장</a>
						</p>
					</li>
				<% } %>
			</ul>
